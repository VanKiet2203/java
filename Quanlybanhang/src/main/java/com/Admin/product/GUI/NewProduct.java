...
public class NewProduct extends javax.swing.JFrame {
    ...
    String color = txtCPU.getText().trim(); // Đổi từ CPU thành Color
    String speed = txtCard.getText().trim(); // Đổi từ Graphics Card thành Speed
    String batteryCapacity = txtRam.getText().trim(); // Đổi từ RAM thành Battery Capacity
    ...
    if (productId.isEmpty() || productName.isEmpty() || color.isEmpty() 
        || speed.isEmpty() || batteryCapacity.isEmpty() || categoryId.isEmpty() 
        || priceStr.isEmpty()) {
    ...
    try {
        ...
        if (price < 0) {
            CustomDialog.showError("Price must be greater than or equal to 0!");
            return;
        }
