...
public class DAOProduct_Stock {
    ...
    public DTOProduct_Stock getProductDetailByID(String productID) {
        ...
        try {
            ...
            String sql = "SELECT p.Product_ID, p.Product_Name, p.Price, ps.Quantity_Stock, p.Category_ID, p.Image, p.Warranty_Period, " +
                         "       p.Motor, p.Battery_Capacity, p.Range_Per_Charge, p.Ride_Mode " +
                         "FROM Product p " +
                         "INNER JOIN Product_Stock ps ON p.Product_ID = ps.Product_ID " +
                         "WHERE p.Product_ID = ?";
            ps = conn.prepareStatement(sql);
            ...
            if (rs.next()) {
                ...
                dto.setCpu(rs.getString("Motor"));
                dto.setRam(rs.getString("Battery_Capacity"));
                dto.setGraphicsCard(rs.getString("Range_Per_Charge"));
                dto.setPrice(rs.getBigDecimal("Price"));
                dto.setWarrantyPeriod(rs.getString("Warranty_Period"));
                dto.setQuantity(rs.getInt("Quantity_Stock"));
                dto.setOperatingSystem(rs.getString("Ride_Mode"));
